from app.analysis.categories import life_info, restaurant, schedule, study
from app.analysis.categories.base import CategoryStage2

# 카테고리 -> 2단계 처리 레지스트리.
# 새 카테고리를 추가하려면: categories/<name>.py 를 만들고 여기에 한 줄 등록.
# 등록되지 않은 카테고리(예: shopping, unknown)는 2단계를 건너뛰고
# 1단계 기본 결과만 반환된다.
CATEGORY_STAGE2: dict[str, CategoryStage2] = {
    ...
    "life_info": CategoryStage2(life_info.build_life_info_prompt, life_info.LifeInfoDetails),
    "restaurant": CategoryStage2(restaurant.build_restaurant_prompt, restaurant.RestaurantDetails),
    # "shopping": 아직 2단계 미구현 (분류만 지원)
}
