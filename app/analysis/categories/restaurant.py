...
서로 다른 가게는 반드시 restaurants 배열의 서로 다른 항목으로 분리하세요.
같은 가게의 메뉴판/외관처럼 여러 원본이 이어진 경우에만 한 항목으로 합치세요.
...
- 한 SOURCE에 서로 다른 가게가 실제로 함께 있으면 그 SOURCE를 각 가게 항목에 포함하세요.
- title과 summary에는 다른 가게의 이름이나 정보를 섞지 마세요.
